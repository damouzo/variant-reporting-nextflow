...
process compareBCIwithGEvar {
    ...
    output:
        tuple val(gene_name), path("${gene_name}_bci_patients_compared_ge_small_variants.tsv"), emit: BciVsGe_tsv
        tuple val(gene_name), path("${gene_name}_bci_patients_compared_ge_small_variants.rds"), emit: BciVsGe_rds
    ...
    if [ ! -f "${gene_name}_bci_patients_compared_ge_small_variants.tsv" ]; then
        echo "Error: TSV output file was not created"
        exit 1
    fi

    if [ ! -f "${gene_name}_bci_patients_compared_ge_small_variants.rds" ]; then

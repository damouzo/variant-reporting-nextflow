// Nextflow process to call the R script for extracting participant IDs from small variants

process extractSmallVarPartID {
    tag { gene_name }
    label 'r_process'

    publishDir "${params.results_dir}/${gene_name}", mode: 'copy'

    input:
        tuple val(gene_name), path(smallvar_annot_file) 
        val labkey_main

    output:
        tuple val(gene_name), path("${gene_name}_small_variants_participantMetadata.rds"), emit: partMet_rds
        path("${gene_name}_small_variants_participantMetadata.tsv"), emit: partMet_tsv
        path("${gene_name}_small_variants_participantID.txt"), emit: partID_txt
        path "versions.yml", emit: versions

    script:
    """    
    echo "Processing: ${gene_name} with annotation file: ${smallvar_annot_file} and labkey: ${labkey_main}"
    extractSmallVarPartID.R ${smallvar_annot_file} ${gene_name} ${labkey_main}

    # Check output files created
    if [ ! -f "${gene_name}_small_variants_participantID.txt" ]; then
        echo "Error: Participants ID output file was not created"
        exit 1
    fi

    if [ ! -f "${gene_name}_small_variants_participantMetadata.tsv" ]; then
        echo "Error: Participants metadata output file was not created"
        exit 1
    fi

    echo "Participants SQL files created successfully"

    cat <<-END_VERSIONS > versions.yml
    "${task.process}":
        r-base: \$(R --version | head -n1 | sed 's/R version //; s/ .*//')
    END_VERSIONS
    """

    stub:
    """
    echo "STUB MODE: Using mock data for ${gene_name}"
    cp "${moduleDir}/mock_data/${gene_name}_small_variants_participantID.txt" "${gene_name}_small_variants_participantID.txt"     
    cp "${moduleDir}/mock_data/${gene_name}_small_variants_participantMetadata.tsv" "${gene_name}_small_variants_participantMetadata.tsv"
    
    # Generate RDS file from TSV using R
    Rscript -e "
    data <- read.table('${gene_name}_small_variants_participantMetadata.tsv', header = TRUE,
                         sep = '\t', stringsAsFactors = FALSE)
    saveRDS(data, '${gene_name}_small_variants_participantMetadata.rds')
    cat('RDS file created successfully from mock TSV data\n')
    "
    
    echo "Mock files ready for ${gene_name}"

    cat <<-END_VERSIONS > versions.yml
    "${task.process}":
        r-base: \$(echo "4.3.3")
    END_VERSIONS
    """
}
...
# Look for all QC metadata plots including GEpatientMetadata variants
metadata_plot_files <- list.files(data_dir, pattern = ".*_QC_CanonicalVar_PartMetadata_*\\.pdf$", full.names = TRUE)
...
if (length(metadata_plot_files) > 0) {
  ...
  if (requireNamespace("pdftools", quietly = TRUE) && requireNamespace("png", quietly = TRUE)) {
    tryCatch({
      ...
      cat('![](', png_filename, '){group="qc-metadata-gallery" width="100%"}\n\n', sep = "")
  ...
  if (length(metadata_plot_files) > 1) {
    for (pdf_idx in 2:length(metadata_plot_files)) {
      ...
      if (requireNamespace("pdftools", quietly = TRUE) && requireNamespace("png", quietly = TRUE)) {
        tryCatch({
          img <- pdftools::pdf_render_page(pdf_path, page = 1, dpi = 300)
          png_filename <- paste0("qc_metadata_", pdf_idx, ".png")
          png::writePNG(img, png_filename)
          
          # Add to lightbox group but hide from main view (without label)
          cat('![](', png_filename, '){group="qc-metadata-gallery" style="display:none;"}\n\n', sep = "")
  ...
  cat("⚠️ QC Participant metadata plots not found.\n")
}
```

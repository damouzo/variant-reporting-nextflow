...
This comprehensive **HTML report** presents a detailed quality control analysis of small germline variants identified in the **{{GENE_NAME}}** 
...
- Comprehensive filtering analysis (basic and custom filters)
...
This section presents comprehensive visualizations of variant quality control metrics, including impact distribution, 
...
This section presents the demographic and clinical characteristics of participants with variants in {{GENE_NAME}}, 
    ...
    basic_stats %>%
      mutate(
        metric = case_when(
          ...
          metric == "total_canonical_nonBenign_MAF<0.001" ~ "Final Filtered Variants (MAF<0.001)",
          TRUE ~ metric
        )
      ) %>%
      gt() %>%
      tab_header(
        ...
        subtitle = "Progressive filtering steps and variant counts"
      ) %>%
      cols_label(
...
# Look for basic filtering plots - try multiple patterns
...
if (length(basic_filter_plot_files) == 0) {
  ...
  basic_filter_plot_files <- list.files(data_dir, pattern = ".*filter_basic.*\\.pdf$", full.names = TRUE)

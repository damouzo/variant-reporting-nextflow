...
# Fallback: if not found in data/, look in current directory  
if (length(basic_filtered_files) == 0) {
  basic_filtered_files <- list.files(".", pattern = "*_struct_variants_filter_basic_filtered\\.rds$", full.names = TRUE)
}
...
if (length(custom_filter_names) > 0) {
  for (filter_name in custom_filter_names) {
    ...
    cat('  custom_filtered_files <- list.files(".", pattern = ".*', filter_name, '_filtered\\\\.rds$", full.names = TRUE)\n', sep = "")
    cat('}\n')
    cat('\n')
    cat('if (length(custom_filtered_files) > 0) {\n')

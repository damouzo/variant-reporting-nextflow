...
sample_cols <- variant_table[, c("Het_samples", "Hom_samples", "Hemi_samples")]
sample_cols <- unique(sample_cols)
sample_vector <- as.character(unlist(sample_cols))
sample_vector <- sample_vector[!is.na(sample_vector) & sample_vector != ""]
split_samples <- unlist(strsplit(sample_vector, ","))
clean_samples <- trimws(split_samples)
all_samples <- unique(clean_samples[clean_samples != ""])
...
for (limit in row_limits) {
  query <- labkey_to_df(sql, labkey_main, limit)
...
saveRDS(participant_metadata, file = paste0(gene_name, "_small_variants_participantMetadata.rds"))

# Save all participant IDs
all_participants <- unique(query$participant_id)
writeLines(all_participants, paste0(gene_name, "_small_variants_participantID.txt"))
